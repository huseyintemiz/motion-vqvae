...
def cal_performance_huseyin(pred,pred2, labels,labels2, ignore_index=None, smoothing=0., tk=1):
    ...
    # pred_id_k_2 = torch.topk(pred2, k=tk, dim=1).indices
    # pred_id2 = pred_id_k_2[:, 0]
    # n_correct2 = (pred_id_k_2 == labels.unsqueeze(1)).any(dim=1).masked_select(mask)
    # acc2 = torch.mean(n_correct2.float()).item()
    ...
    pred_id_k2 = torch.topk(pred2, k=tk, dim=1).indices
    pred_id2 = pred_id_k2[:, 0]
    mask2 = labels2.ne(ignore_index)
    
    n_correct2 = (pred_id_k2 == labels2.unsqueeze(1)).any(dim=1).masked_select(mask2)
    acc2 = torch.mean(n_correct2.float()).item()
    
    
    

    return loss, pred_id,pred_id2, (acc+acc2)/2
