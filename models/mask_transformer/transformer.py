...
class OutputProcess_Bert(nn.Module):
    def __init__(self, out_feats, latent_dim):
        super().__init__()
        self.dense = nn.Linear(int(latent_dim), int(latent_dim))
        self.transform_act_fn = F.gelu
        self.LayerNorm = nn.LayerNorm(int(latent_dim), eps=1e-12)
...
class MaskGroupTransformer(nn.Module):
    def __init__(self, code_dim, cond_mode, latent_dim=256, ff_size=1024, num_layers=8,
                 num_heads=4,num_group=2, dropout=0.1, clip_dim=512, cond_drop_prob=0.1,
                 clip_version=None, opt=None, **kargs):
        super(MaskGroupTransformer, self).__init__()
    ...
    def forward(self, ids, y, m_lens):
        ...
        n_grp,bs, ntokens = ids.shape #huse
        device = ids.device
