...
class MaskGroupTransformerTrainer:
    ...
    def forward(self, batch_data):
        ...
        # (b, n, q)
        code_idx, _ = self.vq_model.encode(motion) # B,T,251 (64,196,251) -> code_idx (64,49,16)
        m_lens = m_lens // 4

        conds = conds.to(self.device).float() if torch.is_tensor(conds) else conds ### conds -> text bunun embedi alinmasi lazim

        # loss_dict = {}
        # self.pred_ids = []
        # self.acc = []
        # code_idx[0,:,:, 0],code_idx[1,:,:, 0] -> burayi ([64x49], [64x49]) -> yerine [2x64x49]olarak gonderebilirim.)
        # _loss, _pred_ids,_pred_ids2, _acc = self.t2m_transformer(code_idx[0,:,:, 0],code_idx[1,:,:, 0], conds, m_lens) # 
        _loss, _pred_ids,_pred_ids2, _acc = self.t2m_transformer(code_idx[:,:,:, 0], conds, m_lens) # 
                                                                 #([2, 64, 49, 8]) grp0__cb_0  (64,49), grp1__cb_0 (64,49)    
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval):
        ...
        # self.opt_t2m_transformer = optim.AdamW(self.t2m_transformer_0.parameters(), betas=(0.9, 0.99), lr=self.opt.lr, weight_decay=1e-5)
        
        self.opt_t2m_transformer = optim.AdamW(chain(self.t2m_transformer_0.parameters(), self.t2m_transformer_1.parameters()), betas=(0.9, 0.99), lr=self.opt.lr, weight_decay=1e-5)
            # chain(self.t2m_transformer_0.parameters(), self.t2m_transformer_1.parameters()), 
        ...
        if 0: ## evaluation_mask_transformer bunun gruop support etmesi lazim
        ...
        while epoch < self.opt.max_epoch:
            ...
            if epoch%10==0: #huseyin
                print("YASOoooooooooooooooooooo")
                best_fid, best_div, best_top1, best_top2, best_top3, best_matching, writer = evaluation_mask_transformer(
...
class MaskTransformerTrainer:
    def __init__(self, args, t2m_transformer_0,t2m_transformer_1, vq_model):
        self.opt = args
        self.t2m_transformer_0 = t2m_transformer_0
        self.t2m_transformer_1 = t2m_transformer_1
        self.vq_model = vq_model
        self.device = args.device
        self.vq_model.eval()
    ...
    def forward(self, batch_data):
        ...
        # (b, n, q)
        code_idx, _ = self.vq_model.encode(motion) #  code_idx -> torch.Size([2, 64, 49, 8])
        m_lens = m_lens // 4
        ...
        # _loss, _pred_ids, _acc = self.t2m_transformer_0(code_idx[..., 0], conds, m_lens) #
        _loss_0, _pred_ids, _acc_0 = self.t2m_transformer_0(code_idx[0,:,:, 0], conds, m_lens) #huseyin
        _loss_1, _pred_ids, _acc_1 = self.t2m_transformer_1(code_idx[1,:,:, 0], conds, m_lens) #huseyin

        _loss = _loss_0 + _loss_1
        _acc = (_acc_0 + _acc_1) / 2
    ...
    def save(self, file_name, ep, total_it):
        t2m_trans_state_dict = self.t2m_transformer_0.state_dict()
        clip_weights = [e for e in t2m_trans_state_dict.keys() if e.startswith('clip_model.')]
        for e in clip_weights:
            del t2m_trans_state_dict[e]
        state = {
    ...
    def train(self, train_loader, val_loader, eval_val_loader, eval_wrapper, plot_eval):
        self.t2m_transformer_0.to(self.device)
        self.t2m_transformer_1.to(self.device)
        ...
        print('Iters Per Epoch, Training: %04d, Validation: %03d' % (len(train_loader), len(val_loader)))
        logs = defaultdict(def_value, OrderedDict())
        if 10:# dissable until 
            
            trans_list = self.t2m_transformer_0,self.t2m_transformer_1
            best_fid, best_div, best_top1, best_top2, best_top3, best_matching, writer = evaluation_mask_transformer(
                self.opt.save_root, eval_val_loader, trans_list, self.vq_model, self.logger, epoch,
                best_fid=100, best_div=100,
                best_top1=0, best_top2=0, best_top3=0,
                best_matching=100, eval_wrapper=eval_wrapper,
                plot_func=plot_eval, save_ckpt=False, save_anim=False
        ...
        while epoch < self.opt.max_epoch:
            self.t2m_transformer_0.train()
            self.t2m_transformer_1.train()
            self.vq_model.eval()
            ...
            for i, batch in enumerate(train_loader):
                ...
                if it % self.opt.save_latest == 0:
                    self.save(pjoin(self.opt.model_dir, 'latest.tar'), epoch, it)
            # gecici
            # self.save(pjoin(self.opt.model_dir, 'latest.tar'), epoch, it)#huseyin
            epoch += 1

            print('Validation time:')
            self.vq_model.eval()
            self.t2m_transformer_0.eval()
            self.t2m_transformer_1.eval()
            ...
            if True: #huseyin # gecici evaluation calismiyor henuz
                
                trans_list = self.t2m_transformer_0,self.t2m_transformer_1
                best_fid, best_div, best_top1, best_top2, best_top3, best_matching, writer = evaluation_mask_transformer(
                    self.opt.save_root, eval_val_loader, trans_list, self.vq_model, self.logger, epoch, best_fid=best_fid,
                    best_div=best_div, best_top1=best_top1, best_top2=best_top2, best_top3=best_top3,
                    best_matching=best_matching, eval_wrapper=eval_wrapper,
                    plot_func=plot_eval, save_ckpt=True, save_anim=(epoch%self.opt.eval_every_e==0)
                )
...
class MaskTransformerTrainer_ORG:
    def __init__(self, args, t2m_transformer, vq_model):
        self.opt = args
        self.t2m_transformer = t2m_transformer
        self.vq_model = vq_model
